...
function Header() {
    ...
    useEffect(() => {
      const handleClickOutside = (event: MouseEvent) => {
        if (menuRef.current && !menuRef.current.contains(event.target as Node) && !hamburgerRef.current?.contains(event.target as Node)) {
          setMenuOpen(false);
        }
      };
  ...
  return (
    <div className='py-1 bg-black text-white grid grid-cols-1 sm:grid-cols-3 items-center fixed w-full top-0 z-50'>
      ...
      {
        menuOpen && (
          <div ref={menuRef} className="sm:hidden absolute top-20 left-0 right-0 bg-black z-50 text-lg font-light flex flex-col items-center gap-4 py-4">
            {navButtons.map((button) => (
              <div key={button.id} onClick={() => {scrollToSection(button.id); setMenuOpen(false)}} className={`${button.id === activeSection ? `${button.id === 'donate' ? donateBtnAvtiveStyle : btnActiveStyle}`: `${button.id === 'donate' ? donateBtnStyle : btnStyle}`}`}>{button.label}</div>
            ))}
          </div>
        )
      }
